...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Tambahkan entri baru (hanya data yang diperlukan untuk leaderboard)
    const entry = {
      tesKey: body.tesKey,
    ...
    // Batasi maksimum 1000 entri (hapus yang paling lama)
    if (existingData.length > 1000) {
      existingData = existingData.slice(existingData.length - 1000);
    }

    const newContent = Buffer.from(JSON.stringify(existingData, null, 2)).toString('base64');

    const putBody = {
      message: `[ujicoba] Hasil tes: ${entry.tesNama} - ${entry.pesertaNama} (${entry.skor}/${entry.totalSoal})`,
      content: newContent,
      ...(sha && { sha })
    };
